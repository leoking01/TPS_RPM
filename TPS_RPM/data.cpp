...
void data_visualize::visualize_origin(
        const   string & file_name, const MatrixXd & X, const MatrixXd & Y,
...
#pragma omp parallel for
    for (int i = 0; i < X_outlier.rows(); i++) {
        ...
        circle(img_origin,
               cv::Point2f(  x.x() - min_x + grid_step
                             , x.y() - min_y + grid_step  ),
               radius_x,
               color_x,
               thickness_x);
...
void data_visualize::visualize_result(
        ...
        , const MatrixXd X_outlier, const MatrixXd & Y_outlier
...
//    const cv::Scalar color(0, 0, 0);
    ...
    PointsShowOnMat  ps;
    ps.show_grid_on_image();
    ps.show_pts_on_image(transform_pts,cv::Scalar(255, 0, 0 ), 4  );
